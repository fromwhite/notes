...
fn main() -> Result<()> {
    let path = String::from(".generated/xor/index.json");
    let posts: Vec<post::Post> = post::read(
        read_to_string(&path).with_context(|| format!("directory {:?} not found", &path))?,
    )?;
    storage::write(posts, String::from(".generated/xor/storage"))?;
